...
关键实现点（逐条搬运自旧实现 `narnat_agent/tools/background/__init__.py`）：
- 完成判定 = 主进程退出 + 管道收口宽限(DRAIN_GRACE)：命令内嵌 `start /b`
  时孙进程继承管道写端永不 EOF，宽限后仍未收口即强制收口，泵线程不会永久阻塞。
...
结构说明：旧实现的模块级槽位表/锁/事件/序号与全局结果目录收敛为
`BackgroundManager` 实例（消灭模块级可变状态）；中断标志不再由本模块
直接读写 bash 模块的类属性，改经 `ShellRuntime` 的显式 API
（`clear_interrupt` / `consume_interrupt`）；杀树与 UTF-8 环境改用
`process.py` 的共享接口。
"""
...
from .process import DRAIN_GRACE, UTF8_ENV, find_executable, kill_proc_tree

__all__ = ["MAX_SLOTS", "BackgroundManager"]
...
TAIL_SUFFIX = ".tail"
READ_CHUNK = 4096                  # 泵线程读块大小（输出分块落盘粒度）
...
COMMAND_DISPLAY_CHARS = 40         # 快照/摘要中命令的展示长度（超出加 …）
...
class _Slot:
    ...
    log_path: str = ""
    fh: Optional[object] = None   # 主文件句柄（wb，泵线程持续写）
...
class _StreamDecoder:
    ...
    策略与前台 `decode_output` 一致：UTF-8 优先、Windows 回退 GBK、兜底
    替换。增量性：单次 read(4KB) 可能在多字节字符中间截断，逐块直接
...
class BackgroundManager:
    """后台任务槽位管理器 —— 提交 / 状态 / 等待 / 取消 / 清理（实例化，无模块级状态）。

    - 槽位表、锁、终态事件、归档序号与结果目录都是实例字段（旧实现为模块级全局）；
    - `prepare()` / `cleanup_all()` 由宿主的会话生命周期调用（预清不杀进程、
      硬清理杀全部运行中任务并可重复调用）；
    - `running_count()` / `running_summary()` 供宿主收尾软提醒使用。
    """

    def __init__(self, runtime) -> None:
        """构造：`runtime` 为 `ShellRuntime`（等待检查点经其消费中断标志）。"""
        self._runtime = runtime
        self._slots: List[_Slot] = [_Slot(id=i) for i in range(1, MAX_SLOTS + 1)]
        self._lock = threading.Lock()             # 槽位表锁（泵线程写文件也在其内）
        self._terminal_event = threading.Event()  # 任一任务进入终态时置位（wait 依赖）
        self._archive_seq = 0                     # 归档文件序号（会话内递增，防同秒覆盖）
        self._base_dir: Optional[str] = None      # 本会话结果目录（首次使用时创建）
    ...
    def _root_dir(self) -> str:
        """结果根目录：首次使用时创建本进程专属临时目录并清扫历史残留。"""
        if self._base_dir is None:
            self._base_dir = tempfile.mkdtemp(prefix=BG_DIR_PREFIX)
            _sweep_stale()
    ...
    def _archive_old(self, slot: _Slot, min_bytes: int) -> str:
        ...
        try:
            ...
            if not has_content:
                return ""
            self._archive_seq += 1
            archived = f"{old_main}.{self._archive_seq}.prev"
    ...
    def submit(self, command: str) -> str:
        ...
        槽位语义：终态(done/failed/cancelled)槽位即释放；复用时旧结果先归档为
        bgN.log.<seq>.prev（永不静默丢失），新任务日志写回 bgN.log——编号即身份，
        AI 只记"bgN 对应 bgN.log"。分配策略：空闲槽优先、其次最旧终态
        （刚完成的结果尽量多留读取窗口期）。
        """
        with self._lock:
            free = [s for s in self._slots if s.status != STATUS_RUNNING]
            if not free:
                running = [s.id for s in self._slots if s.status == STATUS_RUNNING]
                return error_line(
        ...
        archived = self._archive_old(slot, prev_total)
        ...
        for stream in (proc.stdout, proc.stderr):
            threading.Thread(target=self._pump, args=(slot, stream, gen), daemon=True).start()
        # 监控线程：主进程退出 → 管道收口 → 终态
        threading.Thread(target=self._monitor, args=(slot, proc, gen), daemon=True).start()
        msg = (
    ...
    def _pump(self, slot: _Slot, stream, gen: int) -> None:
            ...
            while True:
                ...
                with self._lock:
                    if slot.generation != gen:
                        return
                    self._append_output(slot, text.encode("utf-8"))
        except Exception:
            pass
        finally:
    ...
    def _monitor(self, slot: _Slot, proc, gen: int) -> None:
        ...
        while True:
            ...
            time.sleep(0.02)
        self._finalize(slot, proc, gen, rc)

    def _finalize(self, slot: _Slot, proc, gen: int, rc: int) -> None:
        """落终态：状态/退出码/完成时间、摘除文件句柄、唤醒等待者、落 .tail。"""
        with self._lock:
            if slot.generation != gen:
    ...
    def _append_output(self, slot: _Slot, data: bytes) -> None:
        ...
        if not slot.truncated:
            ...
            if len(data) <= room:
                ...
                self._file_write(slot, data)
                return
            if room > 0:
                self._file_write(slot, data[:room])
                slot.file_bytes += room
            slot.truncated = True
            data = data[room:]
        self._append_tail(slot, data)

    def _file_write(self, slot: _Slot, data: bytes) -> None:
        try:
            slot.fh.write(data)
            slot.fh.flush()  # 实时落盘：运行中即可 Read 增量内容
        except (OSError, ValueError):
            pass

    def _append_tail(self, slot: _Slot, data: bytes) -> None:
    ...
    def _snapshot(self) -> str:
        ...
        lines = []
        with self._lock:
            actives = [s for s in self._slots if s.status]
            for s in actives:
                cmd = s.command[:COMMAND_DISPLAY_CHARS] + (
                    "…" if len(s.command) > COMMAND_DISPLAY_CHARS else ""
                )
                ...
                if s.truncated:
                    line += f"（超限截断，尾部见 {s.log_path}{TAIL_SUFFIX}）"
                line += f"\n    结果: {s.log_path}"
                lines.append(line)
            free_ids = [s.id for s in self._slots if s.status != STATUS_RUNNING]
        if not lines:
            return "[后台任务] 当前无后台任务"
    ...
    def wait(self, timeout: int) -> str:
        ...
        """
        self._runtime.clear_interrupt()  # 入口清零：上轮残留中断不污染本次等待
        with self._lock:
            # clear 与采样原子化：任务完成要么发生在 clear 前（wait 开始前已终态，
            # 本就不在 running 列表），要么发生在采样后（事件置位可被 wait 捕获）
            self._terminal_event.clear()
            before_running = [s.id for s in self._slots if s.status == STATUS_RUNNING]
        ...
        while time.time() < deadline:
            ...
            if self._terminal_event.wait(timeout=0.1):
                break
        with self._lock:
            new_terminal = [
        ...
        # 取消唤醒不宣布事件（兼容怪癖保持：文案回落为「等待{N}秒超时，当前无新完成」）
        events = "；".join(
    ...
    def cancel(self, bg_id: int) -> str:
        """取消任务：杀进程树（后台线程），已产出内容保留在文件里仍可读。"""
        with self._lock:
            slot = next((s for s in self._slots if s.id == bg_id), None)
            if slot is None:
    ...
    def running_count(self) -> int:
        """运行中任务数。"""
        with self._lock:
            return sum(1 for s in self._slots if s.status == STATUS_RUNNING)

    def running_summary(self) -> str:
        """运行中任务的短摘要（宿主软提醒文案用）；无则返回空串。"""
        with self._lock:
            rs = [s for s in self._slots if s.status == STATUS_RUNNING]
            if not rs:
                return ""
            return "、".join(
                f"bg{s.id}({s.command[:COMMAND_DISPLAY_CHARS]})" for s in rs
            )
    ...
    def cleanup_all(self) -> None:
        ...
        with self._lock:
            ...
            self._slots = [_Slot(id=i) for i in range(1, MAX_SLOTS + 1)]
        for p in procs:
            threading.Thread(target=kill_proc_tree, args=(p,), daemon=True).start()
        self._wipe_dir()

    def prepare(self) -> None:
        """会话开始预清：清空上次残留（异常退出场景），确保干净开局。不终止任何进程。"""
        self._wipe_dir()
    ...
    def dispatch(self, command: str, timeout: int, background: bool, bg: object,
                 task_id: object, max_timeout_seconds: Optional[int] = None) -> str:
        """bg 参数操作分发（Shell 工具的后台分发入口）。

        - `background=True`：提交后台任务（command 为空报错）；
        - `bg="status"`：全量快照；
        - `bg="wait"`：等待任意任务完成（秒数取 timeout，0/缺省按 120；受超时上限钳制）；
        - `bg="cancel"`：按 id 取消任务；
        - 其它：未知操作错误行。
        """
        op = str(bg).strip().lower() if bg else ""
        if background:
            if not command:
