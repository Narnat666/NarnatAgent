"""Glob 工具 —— 按模式匹配文件和目录（fd 启发的高性能版本）。

契约来源：specs/tools-file「Glob 参数契约与模式语法」「Glob 结果排序、输出与文案」
「Glob 忽略目录与隐藏项过滤」。

行为搬运自旧实现 `narnat_agent/tools/glob/__init__.py`（花括号展开、glob→正则编译、
scandir 遍历与 top-K 堆、静态前缀拆分、隐藏项策略、忽略目录剪枝、输出与截断提示
逐字一致）；结构重组：模块级常量收敛为全大写常量、`execute` 函数 → `GlobTool`，
`_tool_context.ignore_dirs` → `env.settings`。
"""
...
__all__ = [
    ...
    "compile_pattern",
    "expand_braces",
    "pattern_has_dot_component",
    "split_static_prefix",
    "unescape_braces",
]

# ── 边界参数（原 GlobLimits 类成员收敛为模块常量）──
...
def expand_braces(pattern: str) -> list[str]:
    ...
    单次展开结果上限 `MAX_BRACE_EXPANSIONS`（超出即提前返回已展开项）。
    """
    if "{" not in pattern:
        return [pattern]
    ...
    while i < n:
        ...
        if c == "{":
            ...
            if depth == 0 and start != -1:
                ...
                results: list[str] = []
                for opt in options:
                    for exp in expand_braces(head + opt + tail):
                        if len(results) >= MAX_BRACE_EXPANSIONS:
                            return results
                        results.append(exp)
                return results
        i += 1
...
def unescape_braces(pattern: str) -> str:
    """去除花括号相关转义：\\{ → {, \\} → }, \\, → ,。"""
    result: list[str] = []
    i = 0
    n = len(pattern)
...
def pattern_has_uppercase(pattern: str) -> bool:
...
def compile_pattern(pattern: str) -> re.Pattern[str]:
    ...
    if os.name == "nt":
        ...
        flags = 0 if pattern_has_uppercase(pattern) else re.IGNORECASE
            ...
            with os.scandir(cur_dir) as entries:
                ...
                for entry in entries:
                    ...
                    if is_dir:
                        ...
                        dir_rel_norm = norm_path(dir_rel, _is_nt)
                    ...
                    rel_match = norm_path(rel, _is_nt)
...
def norm_path(path: str, is_nt: bool) -> str:
...
def pattern_has_dot_component(pattern: str) -> bool:
    """pattern 任意路径组件以 . 开头（如 .narnat/**、**/.narnat/*）→ 不过滤隐藏文件。

    对齐 fd 语义：pattern 中显式出现隐藏组件时启用隐藏文件搜索。
    仅剥离前导 ./ 与 /（与 compile_pattern 一致），不剥离组件内部的 .。
    """
    p = pattern.replace("\\", "/")
    while p.startswith("./"):
        p = p[2:]
...
def hidden_files_hint(skip_hidden_files: bool) -> str:
...
def split_static_prefix(pattern: str) -> tuple[str, str]:
    """pattern 拆分: (静态目录, 剩余 pattern)。相对/绝对 pattern 统一适用。

    - 无通配符 → (pattern, "")，调用方做存在性检查
    - 有通配符 → 首个通配符之前最后一个路径分隔符处切分
    - 无法切分 → ("", pattern)，调用方按普通 pattern 处理
    """
    wild_pos = -1
...
GLOB_PARAMS = ("pattern", "path", "max_results")


class GlobTool:
    """Glob 工具实现（`contracts.tool.Tool` 协议）。"""

    name = "Glob"

    def definition(self) -> ToolDefinition:
        """返回 LLM 工具定义（与旧实现 DEFINITION 逐字节等价）。"""
        return GLOB_DEFINITION

    def execute(self, args: dict[str, object], env: ToolEnv | None) -> ToolResult:
        """按模式匹配文件和目录，返回按修改时间倒序的路径列表。"""
        params = bind_params(args, GLOB_PARAMS, ("pattern",))
        ...
        if pattern is None or not str(pattern).strip():
            return ToolResult("[错误: pattern不能为空]")
        pattern = str(pattern).strip()

        root = path or os.getcwd()
        if not os.path.isdir(root):
            # 相对路径解析依赖当前目录（Shell cd 会改变它），报错时带上 cwd 帮 AI 一次定位
            return ToolResult(f"[错误: 目录不存在: {root}（当前目录: {os.getcwd()}）]")

        # AI 可能传字符串类型的数值参数，统一转 int（与 Grep 容错风格一致）
        try:
            max_results = int(max_results) if max_results is not None else 50
        except (TypeError, ValueError):
            return ToolResult("[错误: max_results需为正整数]")

        if max_results <= 0:
            return ToolResult("[错误: max_results需为正整数]")

        # 硬上限防止 OOM（对齐 fd 设计）
        max_results = min(max_results, MAX_HARD_LIMIT)

        # 忽略目录统一来自配置（经 ToolEnv 注入），工具内不再写死
        settings = settings_of(env)
        extra = getattr(settings, "ignore_dirs", None) if settings is not None else None
        ignore_dirs = set(extra) if extra else set()

        # 1. 展开花括号 + 去转义
        raw_patterns = expand_braces(pattern)
        patterns = [unescape_braces(p) for p in raw_patterns]

        # 2. 隐藏文件过滤（对齐 fd：pattern 任意路径组件以 . 开头则不过滤隐藏文件）
        #    注: 不能用 p.lstrip("./")——会把 ".narnat" 开头的 . 误剥离导致判断失效
        skip_hidden_files = not any(pattern_has_dot_component(p) for p in patterns)
        ...
        for p in patterns:
            static_dir, rest = split_static_prefix(p)
            if not rest:
            ...
            if os.path.isabs(static_dir):
                ...
                if not os.path.isdir(joined):
                    ...
                    continue
                search_dir = joined
            res, t = collect(search_dir, [compile_pattern(rest)], ignore_dirs,
        ...
        if root_patterns:
            res, t = collect(root, [compile_pattern(p) for p in root_patterns],
        ...
        if not merged:
            hint = hidden_files_hint(skip_hidden_files)
            if missing_dirs:
                return ToolResult(f"[错误: 目录不存在: {missing_dirs[0]}（当前目录: {os.getcwd()}）]")
            # 带上实际搜索目录，帮 AI 一次定位（Shell cd 会改变当前目录，无匹配常因目录不对）
            # 附隐藏文件提示：让静默的坑变成可见路标，AI 无需背规则
            return ToolResult(f"[无匹配（搜索目录: {root}）]{hint}")
