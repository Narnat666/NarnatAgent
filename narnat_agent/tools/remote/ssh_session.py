"""SSH 交互式会话 —— 单个 SSH 连接的封装（逐行搬运自旧
`narnat_agent/tools/terminal/ssh_session.py`，仅重组结构）。
...
- 哨兵机制: 命令完成后发送独立哨兵 `rc=$?; echo __MARKER__$rc; pwd -P; echo __PWD_MARKER__`
  检测命令完成、捕获退出码与工作目录（延迟注入，避免被命令的 stdin 读取消费）
...
公开面（design D7：跨层直取私有成员改为会话对象公开方法）：
- `is_closed()` 替代外部读 `_channel.closed`；
- `send_interrupt()` 替代外部 `_interrupt.set()` + `_channel.send("\\x03")`；
- `open_sftp()` 替代外部 `_client.open_sftp()`；
- `initialize()` / `initial_output` 替代外部调用 `_initialize()` 与读 `_initial_output`；
- `cwd` / `busy` / `prompt` 状态查询。
"""
...
from ..signal import error_line, rc_line
from .common import clean_output, strip_ansi, truncate_middle

__all__ = ["SSHSession", "truncate_output"]
...
def truncate_output(text: str, max_chars: int) -> str:
    ...
    """
    return truncate_middle(text, max_chars, attach_tags=True)
...
class SSHSession:
    ...
    def __init__(self, host: str, username: str, port: int = 22,
        ...
        # 登录初始输出（由 initialize() 填充；读方经 initial_output 属性访问）
    ...
    def initial_output(self) -> str:
        """连接初始化阶段读到的登录输出（`initialize()` 之后有效）。"""
    ...
    def is_closed(self) -> bool:
        """通道是否已关闭（断线判定；替代外部直取 `_channel.closed`）。"""
        return self._channel.closed

    def send_interrupt(self) -> None:
        """ESC 打断：置本地中断标志（让读循环退出）并向远程发送 Ctrl+C 字节。"""
        self._interrupt.set()
        try:
    ...
    def reconnect(self):
        ...
        with self._reconnect_lock:
            ...
            self.initialize()
            # TCP连接阻塞期间无法中断，返回后 initialize 的读循环会因中断标志立即退出，
    ...
    def execute(self, command: str, timeout: int = 0, max_output_chars: int = 8000) -> str:
        ...
        哨兵机制: 命令完成后发送独立哨兵取退出码/工作目录，用于检测命令结束，
        这是管道基础设施，不是翻译。
        ...
        return truncate_output(result, max_output_chars)
    ...
    def send_input(self, text: str, timeout: int = 0, max_output_chars: int = 8000) -> str:
        ...
        if text == "^C" or text == "\x03":
            ...
            if body:
                return truncate_output(f"{body}\n{tag}\n{self.prompt}", max_output_chars)
            return truncate_output(f"{tag}\n{self.prompt}", max_output_chars)
    ...
    def _update_cwd(self, timeout: int = 3):
        ...
        for i, line in enumerate(lines_all):
            if strip_ansi(line).replace("\r", "").strip().startswith(marker):
                marker_idx = i
                break
    ...
    def _extract_cwd(self, output: str, marker: str, pwd_marker: str) -> Optional[str]:
        ...
        for i in range(marker_idx + 1, len(lines)):
            if strip_ansi(lines[i]).replace("\r", "").strip().startswith(pwd_marker):
    ...
    def _ps1_candidate(text: str) -> bool:
        ...
        last_line = strip_ansi(last_line_raw).replace("\r", "").strip()
        if not last_line:
            return False
    ...
    def _parse_output(self, raw: str, marker: str, pwd_marker: str) -> tuple[str, Optional[str], Optional[int]]:
        ...
        for i, line in enumerate(lines):
            if strip_ansi(line).replace("\r", "").strip().startswith(marker):
                ...
                exit_str = strip_ansi(line).replace("\r", "").strip()[len(marker):]
                try:
        ...
        for l in before_marker.split("\n"):
            ansi_clean = strip_ansi(l).replace("\r", "").strip()
            if f"echo {marker}" in l:
                ...
                if idx >= 0:
                    real_part = l[:idx]
                    if strip_ansi(real_part).replace("\r", "").strip():
    ...
    def _sentinel_line_present(text: str, pwd_marker: str) -> bool:
        ...
        for line in text.split("\n"):
            if strip_ansi(line).replace("\r", "").strip().startswith(pwd_marker):
                return True
        return False
    ...
    def _strip_trailing_prompt(text: str) -> str:
        ...
        if not text:
            return text
        lines = text.split("\n")
        while lines and SSHSession.PROMPT_LINE_RE.match(strip_ansi(lines[-1]).replace("\r", "")):
            lines.pop()
        return "\n".join(lines)
    ...
    def _clean_output(raw: str) -> str:
        ...
        """
        return clean_output(raw, crlf_is_newline=False, strip_markers=True)
