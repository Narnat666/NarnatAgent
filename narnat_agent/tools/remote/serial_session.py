"""串口会话 —— 单个串口连接的封装（逐行搬运自旧
`narnat_agent/tools/serial/serial_session.py`，仅重组结构）。

核心设计:
- 后台 reader 线程持续从串口读取数据到共享 buffer
- 提示符检测: 字符集匹配 + 稳定性采样(3次×100ms)
- 超时兜底: 默认 120s, 超时返回已收集数据（超时即复位忙状态，不保留后台收敛）
- 纯管道原则: AI 发什么就发什么, 不做翻译/注入

公开面：`prompt_info` / `busy` / `is_alive` / `port` / `baudrate` /
`initial_output` / `execute` / `send_input` / `raw_execute` / `kill_active` / `close`。
"""
...
from .common import clean_output, merge_cr_line, strip_ansi, truncate_middle

__all__ = ["SerialSession", "truncate_output"]


def truncate_output(text: str, max_chars: int) -> str:
    """截断输出：保留头部和尾部（尾部常含设备提示符等关键状态），中段提示

    Serial 形态：不做框架标签吸附与 token 估算（Serial 侧错误行不带标签，
    文案为字面量 `[错误: max_output_chars需为正整数]`）。
    """
    return truncate_middle(text, max_chars, attach_tags=False)
...
class SerialSession:
    ...
    # 提示符检测: 以提示符字符结尾, 后跟可选空白（自定义 prompt_pattern 完全替换）
    ...
    def __init__(self, port: str, baudrate: int = 115200,
        ...
        # 参数映射（非法值静默回落默认值：databits/parity/stopbits）
        ...
        self._ser.bytesize = bytesize_map.get(databits, serial.EIGHTBITS)
        self._ser.parity = parity_map.get(parity.upper(), serial.PARITY_NONE)
        self._ser.stopbits = stopbits_map.get(stopbits, serial.STOPBITS_ONE)
    ...
    def prompt_info(self) -> str:
        """会话摘要（`{port} @{baudrate}`；port 与 @ 之间的空格为已发布文案）。"""
    ...
    def send_input(self, text: str, timeout: int = 120,
        ...
        - 其他文本 → 追加行结束符发送，等同于 execute（空闲时也发送，
          与 Terminal 的 input 空闲拒绝语义不同）
        """
    ...
    def raw_execute(self, command: str, timeout: int = 120,
        ...
        - 输出中含大量提示符字符导致误判
        - command 为空 → 纯监听：不发送，仅在 timeout 内收集设备主动输出
        """
        err = self._ensure_ready()
        if err:
            return err
    ...
    def _do_send(self, text: str, timeout: int,
        ...
        if found:
            return truncate_output(self._polish_output(output, text), max_output_chars)
        ...
        if cleaned:
            return truncate_output(f"{cleaned}\n{tag}", max_output_chars)
        return tag
    ...
    def _send_ctrl_c(self, timeout: int, max_output_chars: int) -> str:
        ...
        output, found = self._wait_for_prompt(timeout)
        if found:
            return truncate_output(self._clean_output(output), max_output_chars)
    ...
    def _is_at_prompt(self, text: str) -> bool:
        ...
        if not lines:
            return False
        last_line = merge_cr_line(lines[-1].rstrip())
    ...
    def _clean_output(raw: str) -> str:
        """清洗 ANSI 转义码、`\\r` 覆盖、多余空行（Serial 形态：先归一 CRLF，末尾整体裁剪）"""
        return clean_output(raw, crlf_is_newline=True)
