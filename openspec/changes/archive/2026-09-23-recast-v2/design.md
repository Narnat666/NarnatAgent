...
### D15. 实施后跟进：ESC 打断修复（请求可取消 + 按键事件源）

用户报告「ESC 打断不丝滑」后经探索闭环（`docs/recast/esc_review_notes.md`），在重构成果上做三项结构性修复：

1. **请求发送可取消**（`llm/cancelable.py`）：阻塞发送移入子线程，主流程以 0.05 秒粒度轮询取消标记；取消命中即放弃等待（尽力关闭 + 双侧自毁）。根治"请求处于连接建立阶段时 `close()` 无效、只能等服务端响应头"的慢路径（真机实测 5~11.5s → ≤0.2s）。
2. **按键采集改事件语义**（Windows 主路径）：`ReadConsoleInput` 只认「Esc 键按下」事件，根治字节流"20ms 判定窗口"吞键；中断后继续消费按键，根治"幽灵输入"。
3. **openai 请求级 scope**：中断只影响本次在途请求并标记客户端重建，根治"打断一次后整局对话失败"。

**理由**：三项均为**结构性修复**（消除机制缺陷），非延时/重试式补丁；`specs/interrupt`、`specs/llm` 已同步为有意行为变更（`validate --strict` 通过）。
**备选**：① 仅做界面"正在中断…"即时反馈（治标，慢路径仍在）；② 缩小判定窗口/加快轮询（补丁式，不改语义）。
**影响**：属历史缺陷（新旧实现同等复现），修复后行为更符合用户预期；除 specs 记载的变更点外，其余行为面不变。
...
1. 规格与设计冻结（本 change 的 proposal/specs/design/tasks 完成）→ `openspec validate` 通过。**（已完成）**
2. 逐积木实施（见 tasks.md）：在 `v2/` 开发 + 测试；每块独立验收。**（已完成）**
3. 集成：`v2/` 内完成装配 + 端到端冒烟（headless 真实任务集）。**（已完成）**
4. 切换（单一提交）：删除旧包、上移新包与测试、更新根 `main.py` 导入；运行完整冒烟；`[NN_DONE]` 哨兵与核心命令复验。**（已完成；旧实现留存于 `_old_impl_rollback/` 供对照）**
5. 实施后跟进：ESC 打断修复（D15）与规格同步。**（已完成）**
6. 归档 change（`openspec archive recast-v2`），规格进入 `openspec/specs/` 作为长期金标准。**（已完成）**
