...
流式接收期间系统 SHALL 以不大于 0.05 秒的间隔轮询取消标记；取消标记为真时 SHALL 静默结束本轮（不下发任何事件、不重试）并清理活跃请求句柄。系统 SHALL 维护单一活跃请求句柄：请求发出前指向底层 HTTP 客户端、流就绪后指向响应流、本轮结束（含异常路径）清空；中断触发时 SHALL 关闭该句柄，关闭异常被忽略。中断回调 SHALL 以单槽位注册，未注册或句柄为空时触发为空操作。
