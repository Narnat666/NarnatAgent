...
**实现算法**：
```
1. 编译pattern为正则对象（失败则返回错误）
2. 遍历path下所有文件（受glob过滤，自动跳过.git等忽略目录）
3. 逐文件逐行匹配（Python re模块），若系统有ripgrep(rg)则优先用rg加速
4. 按output_mode格式化输出
...
**用途**：联网搜索，获取实时信息或在线文档。适用于查询最新技术资料、芯片数据手册、协议规范、版本更新日志等本地知识库不覆盖的内容。

**输入**：
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| query | string | 是 | 搜索关键词或自然语言描述 |
| num | int | 否 | 返回搜索结果数量，默认5 |
| lr | string | 否 | 语言限制，如 `lang_en`（英文）、`lang_zh-CN`（简体中文） |

**输出**：搜索结果列表，每条包含标题、URL、摘要文本片段。
...
- **工具自身不重试**：每个工具是无状态原子操作，失败时返回错误信息（含错误类型+上下文），不做内部重试
- **LLM决策下一步**：LLM收到错误信息后自行判断——重试（换参数）、换工具、或告知用户
- **常见失败处理**：
  - Read：文件不存在/权限不足 → 返回错误，LLM可用Glob确认路径后重试
  - Edit：old_string未找到 → 返回相似行提示，LLM先Read确认后重试
  - Bash：超时 → 返回已输出部分+超时提示，LLM决定加大timeout或换方案
  - Grep：非法正则 → 返回错误，LLM修正pattern后重试
