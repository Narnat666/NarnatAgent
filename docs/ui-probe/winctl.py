"""Win32 控制台注入 / 读屏原语（真机验证装置底座）。

用途：把按键事件（KEY_EVENT）写进目标控制台的输入缓冲（无需窗口焦点），
并从目标控制台的屏幕缓冲区读回渲染后的文本（判定"思考中 / 已打断"等状态）。

特点：
- 黑盒：不修改被测程序、不依赖其日志埋点，从外部"像用户一样"注入与观察；
- 免焦点：WriteConsoleInputW 直接写输入队列，目标窗口无需在前台
  （相比 SendInput / keybd_event 的关键优势）；
- 通用：任何有控制台 UI 的程序都适用（narnat 交互模式、nn 等）。
...
class ConsoleIO:
    ...
    def press(self, key: str, hold: bool = True) -> None:
        """注入一次按键（key: esc / backtick / enter / ctrl-c / 单字符）。
